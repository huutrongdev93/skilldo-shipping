		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="">
					...
					<div class="shipping_range_table js_shipping_zone_ward" style="display: none">
						<table class="display table table-striped media-table ">
							<thead>
							<tr>
								<th class="manage-column">Quận huyện</th>
...
<script id="shipping_zone_ward_template" type="text/x-custom-template">
	<tr class="ward_item" data-id="${id}">
		<td class="column-ward">
			<select name="zoneWards[${id}][wards][]" class="form-control js_shipping_zone_select_ward select2-multiple" multiple>${wardOptions}</select>
		</td>
		<td class="column-ward">
			<select name="zoneWards[${id}][fee]" class="form-control js_shipping_zone_select_fee">${feeOptions}</select>
		</td>
		<td class="action column-action">
			<button class="btn btn-red js_shipping_zone_ward_btn_delete" data-id="${id}">{!! Admin::icon('delete') !!}</button>
		</td>
	</tr>
</script>
...
<script id="shipping_zone_template" type="text/x-custom-template">
	<tr class="js_column js_shipping_fee_item" data-id="${id}">
		...
		<td class="column-type">${wardLabel}</td>
		...
		<td class="column-action">
			...
			<button class="btn btn-red js_btn_confirm"
			        ...
			        data-bs-title="Xóa"
			        data-action="delete"
			        data-ajax="Shipping\Ajax\Admin\ShippingZoneAjax::delete"
			        data-id="${id}"
